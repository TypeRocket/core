...
class BaseForm
{
    use FormConnectorTrait, MacroTrait, Attributes, Fieldable;
     ...
     * Instance the From
     *
     * @param Formable|string|array $resource posts, users, comments or options
    ...
    protected function autoConfigModel($model = null, $action = null)
    {
        if (!$this->resource && !$model) {
            ...
            elseif ( isset($comment->comment_ID ) ) {
                ...
                $model = tr_app_class("Models\Comment");
            }
            elseif ( isset( $user_id ) ) {
                $item_id  = $user_id;
                $model = tr_app_class('Models\User');
            }
            elseif ( isset( $taxonomy ) || isset($tag_ID) ) {
                ...
                $item_id  = null;
                $model = tr_app_class("Models\\Option");
            }
    ...
    public function open( $attr = [], $request_params = [], $method = null)
    {
        ...
        $r .= tr_form_hidden_fields($this->method);
        $r .= Html::input('hidden', '_tr_form_prefix', $this->getPrefix());
