...
class Dashboard
{
    ...
    function page_content() {
        $file = apply_filters( 'tr_custom_dashboard_admin_page', get_template_directory() . '/dashboard.php' );
        echo '<div id="typerocket-custom-dashboard" class="wrap">';
        if(file_exists($file)) {
            include $file;
        } else {
            echo 'Create a file named dashboard.php in your active theme\'s folder. Or use the `tr_custom_dashboard_admin_page` filter to change the file used.';
        }
        echo '</div>';
    }
