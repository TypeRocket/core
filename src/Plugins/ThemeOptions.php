...
class ThemeOptions
{
    ...
    public function setup()
    {
        ...
        if (empty($_POST) && !empty($_GET['theme-options']) && !empty($_GET) && $_GET['theme-options'] == 'export') {
            $this->json_export();
        }
    ...
    public function page()
    {
        ...
        echo '<div class="wrap">';
        $file = apply_filters('tr_theme_options_page', __DIR__ . '/views/to-page.php');
        if (file_exists($file)) {
        ...
        if (\TypeRocket\Core\Config::locate('app.debug') == true) {
            ...
            include(__DIR__ . '/views/to-import.php');
        }
        echo '</div>';
    }
    ...
    public function add_sub_menu($name, $link, $root_menu, $id, $meta = false)
    {
        ...
        $wp_admin_bar->add_menu(['parent' => $root_menu, 'id' => $id, 'title' => $name, 'href' => $link, 'meta' => $meta]);
    }
